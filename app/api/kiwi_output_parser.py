...
def extract_info(api_response: dict, user_id: str) -> list[dict]:
    ...
    try:
        for index, flight in enumerate(api_response["data"]):
            if flight["cityTo"] not in seen_cities:
                average_duration = flight["duration"]["total"] / 2
                stop_overs = (
                    len(flight["route"]) - 2
                )  # Subtracting 2 because there's one flight out and one flight back
                ...
                flight_info = {
                    ...
                    "price": {"value": flight["price"], "currency": api_response["currency"]},
    ...
    logger.info(
        "[UserID: %s] Information extraction completed. %s flights left after processing.",
        user_id,
        len(output),
    )
    end_time = time.time()
